...
export function Sidebar(): React.ReactElement {
  ...
  return (
    <aside
      className={`bg-sesi-dark text-white transition-all duration-300 ${
        isOpen ? 'w-64' : 'w-20'
      } flex flex-col`}
    >
                ...
                <li key={entry.id} className="mb-2">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`${!isOpen && 'hidden'} font-medium`}>{entry.title}</span>
                    </div>
                    {isOpen &&
                      ...
                      {entry.items.map((subItem) => {
                        ...
                        return (
                          <li key={subItem.label}>
                            <Link
                              to={subItem.href}
                              className={`flex items-center px-4 py-2 pl-12 transition-colors gap-3 text-sm ${
                                isActive
