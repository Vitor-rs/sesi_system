...
import { useFormativeTemplatesStore } from '../../settings/stores/useFormativeTemplatesStore'
...
export function DisciplinesList(): React.ReactElement {
  ...
  const { templates, fetchTemplates } = useFormativeTemplatesStore()
  const [isModalOpen, setIsModalOpen] = useState(false)

  useEffect(() => {
    fetchDisciplines()
    fetchTemplates()
  }, [fetchDisciplines, fetchTemplates])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {disciplines.map((d) => {
          const exclusiveTemplates = templates.filter((t) => t.disciplineId === d.id)
