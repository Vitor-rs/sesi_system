...
// Helper Components for Floating Labels
...
interface FloatingSelectProps extends React.ComponentProps<'select'> {
  label: string
  icon?: React.ReactNode
}
...
export function StudentsDashboard(): React.ReactElement {
  ...
  return (
    <div className="h-full flex flex-col gap-4">
      ...
      {activeTab === 'students' && (
        <div className="flex-1 min-h-0 flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
          {/* Fixed Area: Stats & Toolbar */}
          <div className="flex-none space-y-4">
            ...
            {/* Toolbar - Compacted (p-3 instead of p-4) */}
            <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-100 flex flex-col xl:flex-row gap-3 items-center justify-between">
              <div className="flex flex-col sm:flex-row items-center gap-3 w-full xl:w-auto flex-1">
                {/* Search */}
                <div className="w-full sm:w-64">
                  <FloatingInput
                    label="Procurar estudante"
                    icon={<Search size={16} />}
                    value={searchTerm}
